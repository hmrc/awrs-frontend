...
trait TestUtilAPI {
  ...
  /**
    * setFormTestDate is used to aid the creating test tuple date data for a form
    *
    * @param preCondition the data already exists for the form
    * @param fieldId      the id of new data field to be added to preCondition
    * @param dateValue    the value of the new date field
    * @return the new test data with (fieldId -> data) appended to the preCondition
    */
  def createFormTestDate(preCondition: Map[String, String], fieldId: String, dateValue: TupleDate): Map[String, String] =
    preCondition ++ generateFormTestData(f"$fieldId.day", dateValue.day)++
      generateFormTestData(f"$fieldId.month", dateValue.month )++
      generateFormTestData(f"$fieldId.year", dateValue.year)
}
