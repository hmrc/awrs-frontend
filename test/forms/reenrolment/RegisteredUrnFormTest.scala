...
import forms.reenrolment.RegisteredUrnForm.{awrsEnrolmentUrnForm, awrsUrn}
  ...
  "AwrsEnrolmentUrnForm" should {
    ...
    "field is more than length mismatch" in {
      form.bind(Map(fieldId -> "XAAW000001234567")).fold(
        formWithErrors => {
          formWithErrors(fieldId).errors.size mustBe 1
          messages(formWithErrors(fieldId).errors.head.message) mustBe messages("awrs.awrsUrn.generic.error", fieldNameInErrorMessage)
        },
        _ => fail("Field should contain errors")
      )
    }

    "field is too short for a AWRS reference number" in {
    ...
    "allow valid submissions" in {
      ...
      assertFormIsValid(form, Map(awrsUrn -> "xZAw  0   0  00099999  \t 9"))
    }
  }
}
