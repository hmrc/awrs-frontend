...
class EmailServiceTest extends AwrsUnitTestTraits
  ...
  "Email Service" should {
    "build the correct request object for the connectors for api4 None user" in {
      ...
      when(mockAWRSNotificationConnector.sendConfirmationEmail(Matchers.eq(expected))(Matchers.any())).thenReturn(true)
    ...
    "get succesful response when sending withdraw email for API8 user" in {
      ...
      when(mockAWRSNotificationConnector.sendWithdrawnEmail(Matchers.eq(expected))(Matchers.any())).thenReturn(true)
    ...
    "get succesful response when sending cancellation email for API10 user" in {
      ...
      when(mockAWRSNotificationConnector.sendCancellationEmail(Matchers.eq(expected))(Matchers.any())).thenReturn(true)
