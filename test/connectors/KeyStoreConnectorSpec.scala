...
class KeyStoreConnectorSpec extends AwrsUnitTestTraits {
  ...
  val testKeyStoreConnector: KeyStoreConnector = new KeyStoreConnector() {
    override val sessionCache: SessionCacheRepository = mockSessionCache
  }

  "KeyStoreConnector" must {
    "fetch saved data from keystore" in {
      when(mockSessionCache.getFromSession[SubscriptionStatusType](DataKey(testUtr))).thenReturn(Future.successful(Some(testSubscriptionStatusTypePendingGroup)))
    ...
    "save data to keystore" in {
      when(mockSessionCache.putSession[SubscriptionStatusType](DataKey(testUtr), testSubscriptionStatusTypePendingGroup)).thenReturn(Future.successful(returnedKeystoreCacheMap))
