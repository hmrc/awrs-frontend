...
class Save4LaterConnectorSpec extends AwrsUnitTestTraits {
  ...
  "ShortLivedCache" must {
    "fetch saved BusinessDetails from save4later" in {
      ...
      when(mockShortLivedCache.fetchAndGetEntry[BusinessCustomerDetails](any(), ArgumentMatchers.eq(key))(any(), any())).thenReturn(Future.successful(Some(reviewDetails)))
    ...
    "save business details into save4later" in {
      ...
      when(mockShortLivedCache.cache(any(), ArgumentMatchers.eq(key), any())(any(),  any())).thenReturn(Future.successful(returnedCacheMap))
    ...
    "fetch all data from save4later by utr" in {
      when(mockShortLivedCache.fetch(any())(any())).thenReturn(Future.successful(Some(returnedCacheMap)))
    ...
    "remove everything from save4later" in {
      when(mockShortLivedCache.remove(any())).thenReturn(Future.successful(()))

      val result = testSave4LaterConnector.removeAll("TEST")

      await(result) mustBe ()
      verify(mockShortLivedCache, times(1)).remove(ArgumentMatchers.eq("TEST"))
    }
