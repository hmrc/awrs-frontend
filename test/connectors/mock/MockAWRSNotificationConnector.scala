...
trait MockAWRSNotificationConnector extends AwrsUnitTestTraits {
  ...
  def setupMockAWRSNotificationConnectorWithOnly(fetchNotificationCache: MockConfiguration[Future[Option[StatusNotification]]] = DoNotConfigure,
    ...
    sendConfirmationEmail ifConfiguredThen (config => when(mockAWRSNotificationConnector.sendConfirmationEmail(Matchers.any())(Matchers.any())).thenReturn(config))
  }
  ...
  def verifyAWRSNotificationConnector(fetchNotificationCache: Option[Int] = None,
    ...
    sendConfirmationEmail ifDefinedThen (count => verify(mockAWRSNotificationConnector, times(count)).sendConfirmationEmail(Matchers.any())(Matchers.any()))
  }
