...
class IndexViewTest extends AwrsUnitTestTraits with ServicesUnitTestFixture {
      ...
      def checkAwrsRefNo(shouldExists: Boolean)(implicit result: Future[Result]): Unit = {
        ...
        shouldExists match {
          case true =>
            ...
            document.select("#awrsRefNo").text should include(AwrsNumberFormatter.format("0123456"))
          case false =>
  ...
  private def showIndexPage(hasAwrs: Boolean,
    ...
    setAuthMocks(Future.successful(uk.gov.hmrc.auth.core.retrieve.~(Enrolments(TestUtil.defaultEnrolmentSet),
