...
//    }
  ...
  def getRow(div: Element): Row = {
    ...
    Row(row1stCol.head.text, row.head.getElementsByClass("govuk-summary-list__value").foldLeft(List[String]()){ (list, x) =>
      list :+ x.text
    })
  }
  ...
  def getSubview(implicit doc: Document, id: String): Subview = {
    ...
    val rows = div.getElementsByClass("govuk-summary-list").foldLeft(List[Row]())((list, x) => list :+ getRow(x))
