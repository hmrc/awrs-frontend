...
trait BusinessTypeController extends AwrsController with AccountUtils {
  ...
  private def standardApi5Journey(authRetrievals: StandardAuthRetrievals)(implicit request: Request[AnyContent]): Future[Result] =
    for {
      subscriptionType <- api5.retrieveApplication(authRetrievals)
