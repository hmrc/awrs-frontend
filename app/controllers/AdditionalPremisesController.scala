...
class AdditionalPremisesController @Inject()(val mcc: MessagesControllerComponents,
                                             ...
                                             implicit val applicationConfig: ApplicationConfig) extends FrontendController(mcc) with JourneyPage with Deletable[AdditionalBusinessPremisesList, AdditionalBusinessPremises] with SaveAndRoutable {
  ...
  override val addNoAnswerRecord: List[AdditionalBusinessPremises] => List[AdditionalBusinessPremises] = (emptyList: List[models.AdditionalBusinessPremises]) => List(AdditionalBusinessPremises(Some("No"), None, None))
