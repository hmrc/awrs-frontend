...
trait StatusManagementService extends LoggingUtils {
  ...
  private def deepFetch(authRetrievals: StandardAuthRetrievals)
                       (implicit hc: HeaderCarrier, request: Request[AnyContent]): Future[StatusReturnType] = {
    lazy val onComplete =
  ...
  private def localFetch(implicit hc: HeaderCarrier, request: Request[AnyContent]): Future[StatusReturnType] =
    for {
  ...
  def retrieveStatus(authRetrievals: StandardAuthRetrievals)(implicit hc: HeaderCarrier, request: Request[AnyContent]): Future[StatusReturnType] =
