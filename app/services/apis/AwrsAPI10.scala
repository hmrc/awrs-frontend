...
trait AwrsAPI10 {
  ...
  def deRegistration(authRetrievals: StandardAuthRetrievals)(implicit request: Request[AnyContent], hc: HeaderCarrier): Future[Option[DeRegistrationType]] =
    for {
  ...
  private def deRegistrationEtmp(authRetrievals: StandardAuthRetrievals)
                                (someDeRegistration: Option[DeRegistration])(implicit request: Request[AnyContent], hc: HeaderCarrier): Future[Option[DeRegistrationType]] =
