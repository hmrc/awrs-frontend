    ...
    data
      ...
      .map(json =>
        json
          .validate[T]
          .fold(
            errors => throw new RuntimeException(s"KeyStore entry for key '$key' was '${Json.stringify(json)}'. Attempt to convert to ${CacheMap.getClass.getName} gave errors: $errors"),
            valid  => valid
          )
      )
}
